...
const GameCanvas: React.FC<GameCanvasProps> = ({ level, onStroke, onHole, disabled = false, isPreview = false }) => {
  ...
  const tRexImgRef = useRef<HTMLImageElement | null>(null);
  ...
  useEffect(() => {
    ...
    trexImg.src = '/assets/moveuspeepolove.png';
    trexImg.onload = () => {
      tRexImgRef.current = trexImg;
    };
  }, [level]);
  ...
  const calculateLayout = useCallback((width: number, height: number) => {
    ...
    if (isPortrait) {
      ...
      // Allocate 160px padding for the top HUD to avoid collision
      const availableH = isPreview ? height : height - 160;
      const widthScale = width / gridWidth;
      const heightScale = availableH / gridHeight;
      ...
      return {
        ...
        y: isPreview ? Math.floor((availableH - scaledHeight) / 2) : 160 + Math.floor((availableH - scaledHeight) / 2) // Shift down vertically below HUD
      };
    } else {
      // In landscape mode: fit within available space
      let availableW = width;
      let availableH = isPreview ? height : height - 84 - 180; // Account for header and footer
  ...
  const update = useCallback(() => {
    ...
    decorationsRef.current.forEach(d => {
        if ((d.type === 'ANDROID' || d.type === 'PENGUIN') && d.vel) {
            ...
            if (distSq < minDist * minDist && distSq > 0) {
                ...
                let pushX = b.pos.x + nx * (minDist - dist);
                let pushY = b.pos.y + ny * (minDist - dist);
                
                // --- STRICTURE Wall Check for Push ---
                let canPush = true;
                wallsRef.current.forEach(w => {
                  if (w.type === 'SQUARE') {
                    if (pushX + b.radius > w.x && pushX - b.radius < w.x + w.w &&
                        pushY + b.radius > w.y && pushY - b.radius < w.y + w.h) {
                      canPush = false;
                    }
                  }
                });
                ...
                if (canPush) {
                  // Clamp to boundaries
                  b.pos.x = Math.max(b.radius, Math.min(gridWidth - b.radius, pushX));
                  b.pos.y = Math.max(b.radius, Math.min(gridHeight - b.radius, pushY));
    ...
    if (b.isMoving) {
        ...
        // --- GLOBAL OOB / VOID RESET ---
        const ballGridX = Math.floor(b.pos.x / TILE_SIZE);
        const ballGridY = Math.floor(b.pos.y / TILE_SIZE);
        
        let isOutOfBounds = false;
        if (ballGridY < 0 || ballGridY >= level.grid.length || 
            ballGridX < 0 || ballGridX >= level.grid[0].length) {
          isOutOfBounds = true;
        } else if (level.grid[ballGridY][ballGridX] === CHAR_VOID) {
          isOutOfBounds = true;
        }

        if (isOutOfBounds) {
          // Play hazard sound and reset to start
          playForeground(getPath("/media/audio/sfx/minigolf/outofholehole.mp3"));
          b.pos.x = startPosRef.current!.x;
          b.pos.y = startPosRef.current!.y;
          b.vel.x = 0;
  ...
  const render = useCallback(() => {
    ...
    decorationsRef.current.forEach(d => {
        if (d.type === 'ANDROID' || d.type === 'PENGUIN') {
            if (d.type === 'ANDROID') {
                if (USE_ANDROID_SVG) {
                    ctx.save();
                ...
                ctx.fillStyle = '#FFFFFF';
                ctx.beginPath();
                ctx.arc(-5.2, -2.5, 0.6, 0, Math.PI * 2);
                ctx.fill();
                ...
                ctx.fillStyle = '#FFFFFF';
                ctx.beginPath();
                ctx.arc(3.8, -2.5, 0.6, 0, Math.PI * 2);
                ctx.fill();
                ...
                ctx.beginPath();
                ctx.arc(0, -7, 10.5, Math.PI * 1.05, Math.PI * 1.95); 
                ctx.ellipse(0, -7, 10.5, 6, 0, 0, Math.PI);
                ctx.fill();
                
                // Beak of the cap (folded edge)
                ctx.fillStyle = '#2196F3'; // Slightly lighter blue for the fold
                ctx.beginPath();
                ...
                ctx.fill();
                // Pom-pom texture/shadow
                ctx.fillStyle = 'rgba(0,0,0,0.1)';
                ctx.beginPath(); ctx.arc(1.5, -19, 2.5, 0, Math.PI * 2); ctx.fill();

                ctx.restore();
            }
        }
 else if (d.type === 'TREX') {
          if (tRexImgRef.current) {
              ctx.save();
              ...
              const imgW = 34;
              const imgH = 34;
              ctx.drawImage(tRexImgRef.current, -imgW/2, -imgH/2, imgW, imgH);
  ...
  useEffect(() => {
    const loop = (timestamp: number) => {
      ...
      const frameThreshold = isPreview ? 1000/15 : FRAME_DURATION;
      
      if (elapsed >= frameThreshold) {
        lastFrameTimeRef.current = timestamp - (elapsed % frameThreshold);
        if (!isPreview) update();
        render();
      }
  ...
  return (
    <div ref={containerRef} className={`w-full overflow-hidden bg-transparent ${isPreview ? 'relative h-full rounded-2xl pointer-events-none' : 'h-full relative'}`}>
      <canvas 
        ...
        className={`touch-none select-none ${isPreview ? '' : 'cursor-crosshair'}`} 

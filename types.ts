...
export interface Decoration {
  type: 'ANDROID' | 'TREX' | 'BOUNCY_PAD' | 'PENGUIN';
  pos: Vector2;
  radius: number;
...
export interface LevelData {
  ...
  requiredPoints?: number; // Minimum points on this specific hole
  requiredTotalPoints?: number; // Minimum accumulated lifetime points
  maxStrokes?: number;
  bonusObjectives?: Array<{
    maxStrokes: number;
    multiplier: number;
  }>;
  maxTime?: number;
  ...
  maxNFTCount?: number;
  exactNFTMatch?: boolean; // If true, user must have EXACTLY requiredNFTCount
}
...
export interface GameState {
  ...
  totalScore: number;
  points: number;
  lastHolePoints: number;
  state: 'START_SCREEN' | 'HOME_SCREEN' | 'LEVEL_SELECT' | 'AIMING' | 'MOVING' | 'SUNK' | 'LEVEL_COMPLETE' | 'GAME_OVER' | 'EDITING' | 'LEVEL_FAILED' | 'PRE_LAUNCH' | 'COUNTDOWN';
  bestScores: Record<number, number>; // Maps level index to best strokes
  levelPoints: Record<number, number>; // Maps level index to points earned
  completedLevels: number[]; // Array of level indices that have been passed
  resetUnlocked: boolean;
  isEligibleToAdvance: boolean;
  performanceGoalMet: boolean;
}
